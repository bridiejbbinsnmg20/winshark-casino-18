...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-e7f8c64c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-e7f8c64c">'+'<span class="tile-face-e7f8c64c tile-front-e7f8c64c"></span>'+'<span class="tile-face-e7f8c64c tile-back-e7f8c64c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e7f8c64c")){return;}
var back=tile.querySelector(".tile-back-e7f8c64c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-e7f8c64c");if(isMine){tile.classList.add("boom-e7f8c64c");}
if(dim){tile.classList.add("dimmed-e7f8c64c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-e7f8c64c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-e7f8c64c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e7f8c64c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-e7f8c64c");void panelEl.offsetWidth;panelEl.classList.add("jolt-e7f8c64c");}
endRound();}
